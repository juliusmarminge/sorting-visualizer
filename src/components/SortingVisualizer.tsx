...
function hasSameValues(arr1, arr2) {
  ...
  for (let i = 0; i < arr1S.length; i++)
    if (arr1S[i] !== arr2S[i]) return false;
  return true;
}
...
class SortingVisualizer extends React.Component {
  ...
  componentDidUpdate(prevProps, prevState, snapshot) {
    if (prevState.barCount !== this.state.barCount) this.generateArray();
  }
  ...
  isSorted() {
    ...
    for (let i = 0; i < barValues.length; i++) {
      if (barValues[i] !== sortedValues[i]) return false;
    }
    return true;
  }
    ...
    return (
      <div className={"container"}>
        <div className="SortingVisualizer">
          ...
          <div
            className={"bars"}
            style={{ width: `${BARS_SZ[0]}%`, height: `${BARS_SZ[1]}%` }}
          >
            {array.map((bar, index) => {
              return (
                <Bar
